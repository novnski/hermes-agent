...
def _host_is_opencode_ai(url: str) -> bool:
    """Return whether *url* is hosted by the OpenCode subscription relay."""
    try:
...
def _opencode_family_for_runtime(provider: str, base_url: str) -> str | None:
    """Resolve an OpenCode family from the provider or trusted relay host."""
    ...
    if _host_is_opencode_ai(base_url):
        return "opencode-go" if "/zen/go" in str(base_url).lower() else "opencode-zen"
    return None
...
def _make_agent(
    ...
    if isinstance(model_override, dict) and model_override.get("model"):
        ...
        if resolution.used_fallback:
            ...
            fresh_base_url = runtime.get("base_url")
            if override_base_url:
            ...
            # OpenCode Go/Zen models may switch between wire families. Never
            # let a stale session row redirect an OpenCode subscription token
            # to another provider's default endpoint; derive the wire from the
            # requested model and retain overrides only on opencode.ai.
            oc_family = _opencode_family_for_runtime(
                str(requested_provider or ""), str(fresh_base_url or "")
            )
            if oc_family is not None:
                from hermes_cli.models import (
                    normalize_opencode_base_url,
                    opencode_model_api_mode,
                )

                runtime["api_mode"] = opencode_model_api_mode(oc_family, model)
                oc_base = (
                    override_base_url
                    if _host_is_opencode_ai(str(override_base_url or ""))
                    else fresh_base_url
                )
                runtime["base_url"] = normalize_opencode_base_url(
                    oc_family, runtime["api_mode"], str(oc_base or "")
                )
    else:
