...
class TestFTS5Search:
    ...
    def test_search_projection_skips_context_enrichment_queries(self, db, monkeypatch):
        ...
        statements = []
        traced_connections = {id(db._conn): db._conn}
        db._conn.set_trace_callback(statements.append)
        original_checkout = db._checkout_read_conn

        def traced_checkout():
            conn = original_checkout()
            if conn is not None and id(conn) not in traced_connections:
                conn.set_trace_callback(statements.append)
                traced_connections[id(conn)] = conn
            return conn
        ...
        try:
            ...
            for conn in traced_connections.values():
