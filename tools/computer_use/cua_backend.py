...
def _resolve_cua_driver_app_path(driver_cmd: str) -> Optional[str]:
    ...
    # PATH lookup commonly returns a symlink into CuaDriver.app. Resolve only
    # that exact path before applying the existing fail-closed bundle check;
    # there is still no /Applications fallback to an unvalidated install.
    try:
